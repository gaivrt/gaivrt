...
- `workers/liuyao-api/` preserves WeChat bearer-token compatibility and adds Turnstile-verified anonymous Web sessions in a Secure, HttpOnly, SameSite cookie. The browser gives the production challenge up to 60 seconds, preserves provider-controlled challenge UI in a safe-area mount, and surfaces Cloudflare error/timeout/unsupported states for retry. Exact-origin CORS allows only the site, production disables the unauthenticated legacy root, and anonymous session creation is limited per HMACed IP/day.
- Main and changed hexagram names use the regular display weight and stay on one horizontal line, including four-character names such as `火雷噬嗑`.
